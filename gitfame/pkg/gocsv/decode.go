...
func readToWithErrorHandler(decoder Decoder, errHandler ErrorHandler, out interface{}) error {
	...
	for i, csvColumnHeader := range headers {
		...
		if fieldInfo1 := getCSVFieldPosition(csvColumnHeader, outInnerStructInfo, curHeaderCount); fieldInfo1 != nil {
			csvHeadersLabels[i] = fieldInfo1
...
func readEachWithoutHeaders(decoder SimpleDecoder, c interface{}) error {
	...
	for {
		...
		outInner := createNewOutInner(outInnerWasPointer, outInnerType)
		for j, csvColumnContent := range line {
			fieldInfo1 := outInnerStructInfo.Fields[j]
			if err := setInnerField(&outInner, outInnerWasPointer, fieldInfo1.IndexChain, csvColumnContent, fieldInfo1.omitEmpty); err != nil { // Set field of struct
